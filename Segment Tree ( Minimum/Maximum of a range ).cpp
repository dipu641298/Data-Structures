...
void build(int in, int l, int r)
{
    if(l == r){
        ...
        t[in] = max(t[in*2],t[in*2+1]);                     /// change here to find minimum
    }
}
...
int query(int in, int pl, int pr, int l, int r)
{
    ...
    int m = (pl + pr)/2;
    return max(query(in*2, pl, m, l, min(m,r)), query(in*2+1, m+1, pr, max(l,m+1), r));         /// change here to find minimum
}
...
void update(int in, int l, int r, int up_in, int val)
{
    if(l==r){t[in] = val;}
        ...
        t[in] = max(t[2*in], t[2*in+1]);            /// change here to find minimum
    }
}
